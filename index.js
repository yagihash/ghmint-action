...
try {
    ...
    const oidcRes = await fetchWithTimeout(`${actionsUrl}&audience=${hostname}`, {
    ...
    const stsRes = await fetchWithTimeout(`https://${hostname}/token`, {
        method: 'POST',
        headers: {
    ...
    appendFileSync(process.env.GITHUB_STATE, `token=${token}\n`);
